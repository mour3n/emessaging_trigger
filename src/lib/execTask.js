...
function* spawnTask(taskInfo) {
    ...
    console.log(`执行任务${taskName}-${taskPid}: ${command} ${taskExecFilePath} ${customUrl}`);
    ...
    childProcessHandleCache[taskName].stdout.on('data', function (data) {
        var logItem = `${moment().format('YYYY-MM-DD HH:mm:ss')} stdout-${taskPid}: ${(Buffer.from(data)).toString()}`;
        fs.appendFileSync(taskLogFile, logItem);
    });

    // stderr输出到任务日志
    childProcessHandleCache[taskName].stderr.on('data', function (data) {
        var logInfo = `${moment().format('YYYY-MM-DD HH:mm:ss')} stderr-${taskPid} ${(Buffer.from(data)).toString()}`;
