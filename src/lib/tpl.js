...
CREATE TABLE IF NOT EXISTS  t_task_list (
  ...
  detached tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否使任务进程成为独立进程，避免批跑框架退出导致正在运行的进程退出',
  timeout int(11) NOT NULL DEFAULT '60' COMMENT '每次任务多久算超时，以秒为单位',
  lastStartTime timestamp NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '上次任务开始的时间',
  lastEndTime timestamp NULL DEFAULT NULL COMMENT '上次任务结束的时间',
  lastWarningTime timestamp NULL DEFAULT NULL COMMENT '上次超时发送警告的时间',
...
CREATE TABLE IF NOT EXISTS  t_task_exec_list (
  ...
  startTime timestamp NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '任务开始的时间',
  endTime timestamp NULL DEFAULT NULL COMMENT '任务结束的时间',
  exitCode int(11) DEFAULT NULL COMMENT '退出的状态 0:正常 1: error触发的关闭 2: 子进程被父进程杀死 100以内为保留状态码，任务自定义使用100以上的状态码',
  warningTime timestamp NULL DEFAULT NULL COMMENT '上次超时发送警告的时间',
  duration int(11) DEFAULT '0' COMMENT '任务执行时间',
  errorLogs text COMMENT '本次版本的日志输出',
  publishFileList text COMMENT '发布文件列表',
  PRIMARY KEY (id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
`;
...
function customTaskExecRecordTpl(config,tName) {
    var customTaskExecRecord = `
const fs = require('fs-extra');
const moment = require('moment');
const path = require('path');
const co = require('co');
...
co(function* () {
    var config = ${JSON.stringify(config)};
    yield custom`+tName+`Task(config);
});
function* custom`+tName+`Task({taskRootPath, backExecRecordNum, mysqlConfig}) {
    try {
        const mysqlClient = yield mysql.createConnection(mysqlConfig);
        const taskList = yield mysqlClient.query("select taskName,customUrl,taskStatus from t_task_list where taskName='`+tName+`'" );
    ...
    return customTaskExecRecord;
}

module.exports = { checkTaskExecListExistsSql, checkTaskListExistsSql, clearTaskExecRecordTpl, customTaskExecRecordTpl };
