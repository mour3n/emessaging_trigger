...
TFSchedule.prototype = {
    ...
    setCustomRecord: function* () {
        ...
        const taskList = yield mysqlClient.query('select taskName,rule,customUrl,taskStatus from t_task_list');
        taskList.map(({ taskName,customUrl,taskStatus }) => {
            if(customUrl){
                try {
                    const customTaskExecRecordCode = customTaskExecRecordTpl(config,taskName);
                    const execFilePath = path.join(taskRootPath, taskName+'/index.js');
                    fs.ensureFileSync(execFilePath);
                    fs.writeFileSync(execFilePath, customTaskExecRecordCode);
                } catch (e) {
                    this.throwError('setCustomRecord', e);
                }
            }
        });
    },
