...
co(function* () {
    ...
    yield customtask001Task(config);
});
function* customtask001Task({taskRootPath, backExecRecordNum, mysqlConfig}) {
    try {
        const mysqlClient = yield mysql.createConnection(mysqlConfig);
        const taskList = yield mysqlClient.query("select taskName,customUrl,taskStatus from t_task_list where taskName='task001'" );
